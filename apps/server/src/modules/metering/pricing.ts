...
export const priceBookSchema = z
  .object({
    llm: z.record(z.string(), llmRateSchema).default({
      // 2026-07-23 model refresh (adr-0004 addendum). Rates verified at swap
      // time: gpt-5.4-mini $0.75/$4.50 per 1M (OpenAI pricing page),
      // gemini-3.5-flash-lite $0.30/$2.50 per 1M. Old model ids are DROPPED —
      // costs stamp at write time, so historical usage_events keep the price
      // they were written with; only NEW calls need current rates.
